...
class CountDownAgent(BaseChatAgent):
    def __init__(self, name: str, prompt: str, model_client: ChatCompletionClient):
        """
        初始化CountDownAgent。
    ...
    async def on_messages_stream(
        ...
        async for chunk in self.model_client.create_stream(
            [UserMessage(content=self.prompt, source="user")],
            # tools=tools,
            ...
            if isinstance(chunk, CreateResult):
                ...
                yield ModelClientStreamingChunkEvent(content=chunk, source=self._name, full_message_id=message_id)
            else:
...
async def run_countdown_agent() -> None:
    ...
    """
    # 创建一个倒计时代理实例
    countdown_agent = CountDownAgent(name="countdown",prompt="你好",model_client=model_client)
