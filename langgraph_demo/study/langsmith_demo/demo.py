...
def orchestrator(state: State):
    ...
    try:
        ...
        # 处理结构化输出
        sections = getattr(report_sections, 'sections', None)
        if not sections:
            # 如果结构化输出失败，创建默认章节
            sections = [
                Section(name="概述", description="主题的基本介绍和背景"),
                Section(name="现状分析", description="当前情况的详细分析"),
                Section(name="挑战与机遇", description="面临的挑战和潜在机遇"),
                Section(name="解决方案", description="具体的解决方案和建议"),
                Section(name="总结与展望", description="总结和未来展望")
            ]
        
        print(f"📋 已生成 {len(sections)} 个章节的计划")
        return {"sections": sections}
        
    except Exception as e:
        print(f"⚠️ 协调者规划过程中出现错误: {e}")
        # 创建默认章节作为备选方案
        default_sections = [
            Section(name="概述", description="主题的基本介绍和背景"),
            Section(name="分析", description="深入分析和讨论"),
            Section(name="总结", description="总结和结论")
        ]
        print(f"📋 使用默认计划，共 {len(default_sections)} 个章节")
...
def llm_call(state: WorkerState):
    ...
    section_content = llm.invoke([
        SystemMessage(content="你是一个专业的报告撰写专家。请根据提供的章节名称和描述撰写详细的内容。使用代码格式，内容要专业、详细且有条理。"),
...
def create_debug_graph():
    """为 langgraph.json 创建调试图的入口函数"""
    return build_orchestrator_worker_graph()
...
def run_simple_example():
    ...
    try:
        ...
        config: RunnableConfig = {
            "configurable": {
                "thread_id": "orchestrator-worker-demo"
            },
        ...
        # 构建输入状态
        input_state: State = {"topic": topic, "sections": [], "completed_sections": [], "final_report": ""}
